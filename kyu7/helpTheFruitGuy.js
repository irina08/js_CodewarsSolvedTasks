...
where some fruits are rotten. He wants to replace all the rotten pieces of
fruit with fresh ones. For example, given ["apple","rottenBanana","apple"]
...
function removeRotten(bagOfFruits){
  ...
  for(let i = 0; i < bagOfFruits.length; i++) {
    if(bagOfFruits[i].includes('rotten'))
      bagOfFruits[i] = bagOfFruits[i].toLowerCase().replace('rotten', '');
  }
  return bagOfFruits;
}
