...
to the bands on a scale from 1 to 50 for three categories: Song Heaviness,
Originality, and Members' outfits.
...
32 for Originality and only 18 for Outfits. Bob listens to Slayer and has gotten
a good 48 for Heaviness, 25 for Originality and a rather honest 40 for Outfits.
The total score should be followed by a colon : and by one of the following
quotes: if Alice's band wins: Alice made "Kurt" proud!
...
function solve(a, b) {
  ...
  if (count1 > count2) {
    return `${count1}, ${count2}: Alice made "Kurt" proud!`
  }
  else if (count1 < count2) {
    return `${count1}, ${count2}: Bob made "Jeff" proud!`
  }
  else {
    return `${count1}, ${count2}: that looks like a "draw"! Rock on!`
  }
}